				...
				throws InterruptedException, InvocationTargetException
			{
				try
				{
					...
					for (int i = 0; i < resources.length; i++)
					{
						...
						{	
							IEditorPart editor = ClearcasePlugin.getDefault().getWorkbench()
								.getActiveWorkbenchWindow()
								.getActivePage().findEditor(new FileEditorInput((IFile) resource));
							if (editor != null && editor.isDirty())
							{
								boolean answer = MessageDialog.openQuestion(shell, "Resource dirty", "The resource " + resource.toString() + " is dirty, do you wish to save before checking in?");
								if (answer)
								{
									editor.doSave(null);
								}
