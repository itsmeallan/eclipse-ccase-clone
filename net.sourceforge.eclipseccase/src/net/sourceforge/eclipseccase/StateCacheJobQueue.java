...
class StateCacheJobQueue extends Job {
	...
	private static final String MESSAGE_QUEUE_NAME = Messages.getString("StateCacheJobQueue.jobLabel"); //$NON-NLS-1$
	...
	protected IStatus run(IProgressMonitor monitor) {
		// synchronized in case update starts during checkCancel
		synchronized (this) {
	 ...
	 */
	private void executePendingJobs(IProgressMonitor monitor) throws CoreException, OperationCanceledException {
		...
		try {
			...
			while (!priorityQueue.isEmpty()) {
				...
				if (null != job.getStateCache().getResource()) {
					monitor.subTask(Messages.getString("StateCacheJobQueue.task.refresh") //$NON-NLS-1$
	...
	synchronized void interrupt() {
		...
		case RUNNING:
			...
			if (interrupted && ClearcasePlugin.DEBUG_STATE_CACHE) {
				ClearcasePlugin.trace("[StateCache] update job was interrupted: " + Thread.currentThread().getName()); //$NON-NLS-1$
