...
import net.sourceforge.eclipseccase.ui.panel.IDialogPanel;

import net.sourceforge.eclipseccase.ui.panel.IDialogManager;
...
public class DefaultDialog extends MessageDialog implements IDialogManager {
	...
	protected Control createInfoPanel(Composite parent) {
		...
		this.mainLabelFont = new Font(getDisplay(), fData);
	...
	protected Image findImage(String imagePath) {
	    ...
	    //ClearCasePlugin plugin = ClearCasePlugin.getDefault();
		ImageDescriptor descriptor = getImageDescriptor(imagePath);		
	   // ImageDescriptor descriptor = reg.getImageDescriptor(imagePath == null ? "icons/wizards/newconnect.gif" : imagePath); //$NON-NLS-1$
	    if (descriptor == null) {
	...
	//Added private 
	private static ImageDescriptor getImageDescriptor(String string) {
