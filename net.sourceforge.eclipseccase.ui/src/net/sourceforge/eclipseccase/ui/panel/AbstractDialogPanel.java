...
public abstract class AbstractDialogPanel implements IDialogPanel, IValidationManager {
    ...
    public final Point getPrefferedSize() {
		int width = SIZE_PREFERRED_WIDTH;
		int height = SIZE_PREFERRED_HEIGHT;
...
//	}
    ...
    public static String makeToBeOperatedMessage(IResource []resources) {
		String message;
		if (resources.length == 1) {
			message = Messages.getString("RepositoryTreePanel_Message_Single");
		}
		else if (resources.length < 5) {
			message = Messages.getString("RepositoryTreePanel_Message_UpTo4");
		}
		else {
			message = Messages.getString("RepositoryTreePanel_Message_Multi");
		}
		
		return Messages.bind(message, FileHelper.getNamesListAsString(resources));
