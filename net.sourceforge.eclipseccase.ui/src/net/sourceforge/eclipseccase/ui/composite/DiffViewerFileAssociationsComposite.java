 ...
 *    Mikael Petterson - adaptations 
 *******************************************************************************/
package net.sourceforge.eclipseccase.ui.composite;

import net.sourceforge.eclipseccase.ui.dialogs.DefaultDialog;
import net.sourceforge.eclipseccase.ui.panel.DiffViewerVariablesPanel;
import net.sourceforge.eclipseccase.ui.panel.EditFileAssociationsPanel;
import net.sourceforge.eclipseccase.ui.preferences.*;
import net.sourceforge.eclipseccase.ui.preferences.DiffViewerSettings.IDiffViewerChangeListener;
import net.sourceforge.eclipseccase.ui.preferences.DiffViewerSettings.ResourceSpecificParameterKindEnum;
import net.sourceforge.eclipseccase.ui.preferences.DiffViewerSettings.ResourceSpecificParameters;
import net.sourceforge.eclipseccase.ui.utility.ColumnedViewerComparator;
import net.sourceforge.eclipseccase.ui.utility.SwtHelper;
import net.sourceforge.eclipseccase.ui.verifier.IValidationManager;
...
/**
 * 
 * File associations for diff/merge viewer It associates either file extension
 * or file mime type with external diff/merge program
 * 
 * @author Igor Burilo
 */
public class DiffViewerFileAssociationsComposite extends Composite {
	...
	protected void createParametersPreview(Composite parent) {
		...
		diffGroup.setText(Messages.getString("DiffViewerExternalProgramComposite_DiffProgramArguments_Label"));

		this.diffParametersText = new Text(diffGroup, SWT.BORDER | SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.WRAP);
		data = new GridData(GridData.FILL_HORIZONTAL);
		data.heightHint = SwtHelper.convertHeightInCharsToPixels(this.diffParametersText, 5);
		...
		mergeGroup.setText(Messages.getString("DiffViewerExternalProgramComposite_MergeProgramArguments_Label"));

		this.mergeParametersText = new Text(mergeGroup, SWT.BORDER | SWT.MULTI | SWT.H_SCROLL | SWT.V_SCROLL | SWT.WRAP);
		data = new GridData(GridData.FILL_HORIZONTAL);
		data.heightHint = SwtHelper.convertHeightInCharsToPixels(this.mergeParametersText, 5);
	...
	protected void createFileAssociationsTable(Composite parent) {
		...
		column.setText(Messages.getString("DiffViewerFileAssociationsComposite_ExtensionMimeType_Column"));
		column.addSelectionListener(comparator);

		column = new TableColumn(table, SWT.NONE);
		column.setText(Messages.getString("DiffViewerFileAssociationsComposite_DiffProgramPath_Column"));
		column.addSelectionListener(comparator);

		column = new TableColumn(table, SWT.NONE);
		column.setText(Messages.getString("DiffViewerFileAssociationsComposite_MergeProgramPath_Column"));
		column.addSelectionListener(comparator);
	...
	protected void createButtonsControls(Composite parent) {
		...
		// Dialog framework used for all buttons.
		...
		addButton.setLayoutData(data);
		addButton.setText(Messages.getString("DiffViewerFileAssociationsComposite_Add_Button"));
		...
		this.editButton.setText(Messages.getString("DiffViewerFileAssociationsComposite_Edit_Button"));
		...
		this.removeButton.setText(Messages.getString("DiffViewerFileAssociationsComposite_Remove_Button"));

		Button variablesButton = new Button(composite, SWT.PUSH);
		data = new GridData(GridData.FILL_HORIZONTAL);
		data.widthHint = DefaultDialog.computeButtonWidth(this.addButton);
		variablesButton.setLayoutData(data);
		variablesButton.setText(Messages.getString("DiffViewerExternalProgramComposite_Variables_Button"));
