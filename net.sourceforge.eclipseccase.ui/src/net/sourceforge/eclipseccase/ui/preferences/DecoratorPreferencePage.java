 ...
 */
public class DecoratorPreferencePage extends
        FieldEditorPreferencePageWithCategories implements
    ...
    private final class PreviewContentProvider implements ITreeContentProvider {
        ...
        public Object[] getChildren(Object parentElement) {
            if (parentElement == ResourcesPlugin.getWorkspace().getRoot()) {
                ...
                return new String[] { PREVIEW_FOLDER, PREVIEW_LINKED_FOLDER,
                        PREVIEW_CHECKEDIN_TXT, PREVIEW_CHECKEDOUT_JAVA,
                        PREVIEW_EDITEDSOMEWHERE_JAVA, PREVIEW_IGNORED_TXT,
                        PREVIEW_HIJACKEDARCHIVE_ZIP, PREVIEW_UNKNOWNSTATE_TXT,
                        PREVIEW_VIEWPRIVATE_TXT };
            } else if (parentElement.equals(PREVIEW_FOLDER)) {
                return new String[] { PREVIEW_CHECKEDIN_TXT,
                        PREVIEW_EDITEDSOMEWHERE_JAVA, PREVIEW_VIEWPRIVATE_TXT };
            } else {
                return new Object[0];
            }
        }
     ...
     * shown
     */
    private final class PreviewLabelProvider extends LabelProvider implements
        ...
        private ImageDescriptor getElementBackgroundImage() {
            ...
            String customImage = imageClearCaseElementsBackground
                    .getStringValue();
        ...
        public Image getImage(Object element) {
            Image baseImage;
            if (element.equals(PREVIEW_PROJECT)) {
                baseImage = PlatformUI.getWorkbench().getSharedImages()
                        .getImage(SharedImages.IMG_OBJ_PROJECT);
            } else if (element.equals(PREVIEW_FOLDER)
                    || element.equals(PREVIEW_LINKED_FOLDER)) {
                baseImage = PlatformUI.getWorkbench().getSharedImages()
            ...
            if (null == overlay && null == background) return baseImage;

            // Otherwise
            ImageDescriptor[] overlays = null != overlay ? new ImageDescriptor[] { overlay }
                    : new ImageDescriptor[0];
            int[] locations = null != overlay ? new int[] { OverlayIcon.BOTTOM_RIGHT }
                    : new int[0];
            ImageDescriptor[] backgrounds = null != background ? new ImageDescriptor[] { background }
                    : null;
            ImageDescriptor resultImageDescriptor = new OverlayIcon(baseImage,
                    overlays, locations, backgrounds, new Point(baseImage
                            .getBounds().width, baseImage.getBounds().height));
            if (imageCache == null) {
        ...
        public String getText(Object element) {
            ...
            if (element.equals(PREVIEW_IGNORED_TXT)) {
                ...
                // edited in another view or branch
                if (decorateElementStatesWithTextPrefix.getBooleanValue()) {
                    prefix = prefixResourcesEditedBySomeoneElse
                            .getStringValue();
                }
            } else if (element.equals(PREVIEW_UNKNOWNSTATE_TXT)) {
                // unknown state
    ...
    /** category 'General' */
    private static final String CAT_GENERAL = PreferenceMessages
            .getString("DecoratorPreferencePage.category.general"); //$NON-NLS-1$

    /** category 'Icons' */
    private static final String CAT_IMAGES = PreferenceMessages
            .getString("DecoratorPreferencePage.category.images"); //$NON-NLS-1$

    /** category 'Text' */
    private static final String CAT_TEXT = PreferenceMessages
            .getString("DecoratorPreferencePage.category.text"); //$NON-NLS-1$

    /** array with all categories */
    private static final String[] CATEGORIES = new String[] { CAT_TEXT,
            CAT_IMAGES, CAT_GENERAL };
    ...
    private static final String PREVIEW_CHECKEDIN_TXT = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.checkedInFile"); //$NON-NLS-1$

    private static final String PREVIEW_CHECKEDOUT_JAVA = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.checkedOutFile"); //$NON-NLS-1$

    private static final String PREVIEW_EDITEDSOMEWHERE_JAVA = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.editedBySomeoneElseFile"); //$NON-NLS-1$

    private static final String PREVIEW_FOLDER = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.folder"); //$NON-NLS-1$

    private static final String PREVIEW_HIJACKEDARCHIVE_ZIP = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.hijackedFile"); //$NON-NLS-1$

    private static final String PREVIEW_IGNORED_TXT = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.ignoredFile"); //$NON-NLS-1$

    private static final String PREVIEW_LINKE_TARGET = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.linkTarget"); //$NON-NLS-1$

    private static final String PREVIEW_LINKED_FOLDER = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.linkedFolder"); //$NON-NLS-1$

    private static final String PREVIEW_PROJECT = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.project"); //$NON-NLS-1$

    private static final String PREVIEW_UNKNOWNSTATE_TXT = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.unknownStateFile"); //$NON-NLS-1$

    private static final String PREVIEW_VERSION_1 = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.version1"); //$NON-NLS-1$

    private static final String PREVIEW_VERSION_2 = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.version2"); //$NON-NLS-1$

    private static final String PREVIEW_VERSION_3 = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.version3"); //$NON-NLS-1$

    private static final String PREVIEW_VERSION_4 = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.version4"); //$NON-NLS-1$

    private static final String PREVIEW_VIEW_TAG = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.viewTag"); //$NON-NLS-1$

    private static final String PREVIEW_VIEWPRIVATE_TXT = PreferenceMessages
            .getString("DecoratorPreferencePage.preview.viewPrivateFile"); //$NON-NLS-1$

    static {
        IMG_DESC_DIRTY = new CachedImageDescriptor(
                TeamImages
                        .getImageDescriptor(org.eclipse.team.ui.ISharedImages.IMG_DIRTY_OVR));
        IMG_DESC_CHECKED_IN = new CachedImageDescriptor(
                TeamImages
                        .getImageDescriptor(org.eclipse.team.ui.ISharedImages.IMG_CHECKEDIN_OVR));
        IMG_DESC_CHECKED_OUT = new CachedImageDescriptor(
                TeamImages
                        .getImageDescriptor(org.eclipse.team.ui.ISharedImages.IMG_CHECKEDOUT_OVR));
        ...
        IMG_DESC_ELEMENT_BG = new CachedImageDescriptor(ClearcaseImages
                .getImageDescriptor(ClearcaseImages.IMG_ELEMENT_BG));
    }
    ...
    public DecoratorPreferencePage() {
        ...
        setDescription(PreferenceMessages
                .getString("DecoratorPreferencePage.description")); //$NON-NLS-1$
    }
    ...
    protected void createFieldEditors() {
        // general
        dirtyFolders = new MasterBooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_FOLDERS_DIRTY,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.folders.dirty"), //$NON-NLS-1$
                getFieldEditorParent(CAT_GENERAL));
        addField(dirtyFolders);

        dirtyFoldersWithViewPrivate = new BooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_FOLDERS_CONTAINING_VIEW_PRIVATE_DIRTY,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.folders.dirty.withViewPrivate"), getFieldEditorParent(CAT_GENERAL));//$NON-NLS-1$
        addField(dirtyFoldersWithViewPrivate);
        dirtyFolders.addSlave(dirtyFoldersWithViewPrivate);

        // image decoration

        decorateClearCaseElements = new MasterBooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_CLEARCASE_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.clearCaseElements"), getFieldEditorParent(CAT_IMAGES)); //$NON-NLS-1$
        addField(decorateClearCaseElements);

        customClearCaseElementsBackground = new MasterBooleanFieldEditor(
                ClearcaseUIPreferences.IMAGE_CLEARCASE_ELEMENTS_BACKGROUND_CUSTOM,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.customClearCaseElementsBackground"), getFieldEditorParent(CAT_IMAGES)); //$NON-NLS-1$
        decorateClearCaseElements.addSlave(customClearCaseElementsBackground);
        addField(customClearCaseElementsBackground);

        imageClearCaseElementsBackground = new FileFieldEditor(
                ClearcaseUIPreferences.IMAGE_CLEARCASE_ELEMENTS_BACKGROUND,
                PreferenceMessages
        ...
        decorateCheckedIn = new BooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_CHECKED_IN_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.checkedIn"), getFieldEditorParent(CAT_IMAGES)); //$NON-NLS-1$
        addField(decorateCheckedIn);

        decorateEdited = new BooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_EDITED_ELEMENTS,
                PreferenceMessages.getString("DecoratorPreferencePage.edited"), getFieldEditorParent(CAT_IMAGES)); //$NON-NLS-1$
        addField(decorateEdited);

        decorateHijacked = new BooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_HIJACKED_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.hijacked"), //$NON-NLS-1$
                getFieldEditorParent(CAT_IMAGES));
        addField(decorateHijacked);

        decorateViewPrivate = new BooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_VIEW_PRIVATE_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.viewPrivate"), //$NON-NLS-1$
                getFieldEditorParent(CAT_IMAGES));
        addField(decorateViewPrivate);

        decorateUnknown = new BooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_UNKNOWN_ELEMENTS,
        ...
        addViewInfoToProjects = new BooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_PROJECTS_WITH_VIEW_INFO,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.addViewInfoToProjects"), //$NON-NLS-1$
                getFieldEditorParent(CAT_TEXT));
        addField(addViewInfoToProjects);

        addVersionInfo = new BooleanFieldEditor(
                ClearcaseUIPreferences.DECORATE_ELEMENTS_WITH_VERSION_INFO,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.appendVersionInfoToResources"), //$NON-NLS-1$
                getFieldEditorParent(CAT_TEXT));
        ...
        decorateElementStatesWithTextPrefix = new MasterBooleanFieldEditor(
                ...
                PreferenceMessages
                        .getString("DecoratorPreferencePage.prependResourceNamesWithStateInfoFor"), //$NON-NLS-1$
                getFieldEditorParent(CAT_TEXT));
        addField(decorateElementStatesWithTextPrefix);

        prefixCheckedInResources = new StringFieldEditor(
                ClearcaseUIPreferences.TEXT_PREFIX_CHECKED_IN_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.checkedInResources"), 4, getFieldEditorParent(CAT_TEXT));//$NON-NLS-1$
        addField(prefixCheckedInResources);
        decorateElementStatesWithTextPrefix.addSlave(prefixCheckedInResources);

        prefixDirtyResources = new StringFieldEditor(
                ClearcaseUIPreferences.TEXT_PREFIX_DIRTY_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.dirtyResources"), 4, getFieldEditorParent(CAT_TEXT));//$NON-NLS-1$
        addField(prefixDirtyResources);
        decorateElementStatesWithTextPrefix.addSlave(prefixDirtyResources);

        prefixHijackedResources = new StringFieldEditor(
                ClearcaseUIPreferences.TEXT_PREFIX_HIJACKED_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.hijackedResources"), 4, getFieldEditorParent(CAT_TEXT));//$NON-NLS-1$
        addField(prefixHijackedResources);
        decorateElementStatesWithTextPrefix.addSlave(prefixHijackedResources);

        prefixViewPrivateResources = new StringFieldEditor(
                ClearcaseUIPreferences.TEXT_PREFIX_VIEW_PRIVATE_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.viewPrivateResources"), 4, getFieldEditorParent(CAT_TEXT));//$NON-NLS-1$
        addField(prefixViewPrivateResources);
        decorateElementStatesWithTextPrefix
                .addSlave(prefixViewPrivateResources);

        prefixResourcesEditedBySomeoneElse = new StringFieldEditor(
                ClearcaseUIPreferences.TEXT_PREFIX_EDITED_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.resourcesEditedBySomeoneElse"), 4, //$NON-NLS-1$
                getFieldEditorParent(CAT_TEXT));
        addField(prefixResourcesEditedBySomeoneElse);
        decorateElementStatesWithTextPrefix
                .addSlave(prefixResourcesEditedBySomeoneElse);

        prefixResourcesWithUnknownState = new StringFieldEditor(
                ClearcaseUIPreferences.TEXT_PREFIX_UNKNOWN_ELEMENTS,
                PreferenceMessages
                        .getString("DecoratorPreferencePage.resourcesWithUnknownState"), 4, //$NON-NLS-1$
                getFieldEditorParent(CAT_TEXT));
        addField(prefixResourcesWithUnknownState);
        decorateElementStatesWithTextPrefix
                .addSlave(prefixResourcesWithUnknownState);
    }
    ...
    protected String getDescription(String category) {
        if (CAT_GENERAL.equals(category))
            return PreferenceMessages
                    .getString("DecoratorPreferencePage.category.general.description"); //$NON-NLS-1$
        if (CAT_IMAGES.equals(category))
            return PreferenceMessages
                    .getString("DecoratorPreferencePage.category.images.description"); //$NON-NLS-1$
        if (CAT_TEXT.equals(category))
            return PreferenceMessages
                    .getString("DecoratorPreferencePage.category.text.description"); //$NON-NLS-1$
