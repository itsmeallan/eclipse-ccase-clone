...
public abstract class ClearcaseViewPart extends ResourceNavigator implements IResourceStateListener, IResourceChangeListener {
	...
	protected void findResources(IProject[] clearcaseProjects, IElementCollector collector, IProgressMonitor monitor) {
		try {
			...
			// retry later if the state cache is not ready yet
			if (!StateCacheFactory.getInstance().isInitialized()) {
				monitor.subTask("Waiting for loading the state cache");
				UIJob job = new UIJob(getViewSite().getShell().getDisplay(), "Refresh Checkouts View") {

					public IStatus runInUIThread(IProgressMonitor monitor) {
						try {
							monitor.beginTask("Refreshing Checkouts View", 100);
							refresh();
						} finally {
							monitor.done();
						}
	...
	public void setWorkingSet(IWorkingSet workingSet) {
		if (null != workingSet && workingSet.isAggregateWorkingSet() && workingSet.isEmpty())
			getContentProvider().setWorkingSet(null);
		else
